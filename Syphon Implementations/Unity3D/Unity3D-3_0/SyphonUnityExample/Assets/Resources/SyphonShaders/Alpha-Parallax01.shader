Shader "Syphon/Transparent/Parallax Diffuse" {
Properties {
...
void main ()
{
  ...
  vec4 c;
  vec2 tmpvar_55;
  tmpvar_55 = tmpvar_2.zw;
  vec2 _ret_val_i0;
  vec3 v;
  vec3 tmpvar_67;
  tmpvar_67 = normalize (gl_TexCoord[1].xyz);
  v = tmpvar_67;
  v.z = (vec3((tmpvar_67.z + 0.42))).z;
  _ret_val_i0 = (((texture2D (_ParallaxMap, tmpvar_55).w * _Parallax) - (_Parallax / 2.0)) * (v.xy / v.z));
  vec4 tmpvar_74;
  tmpvar_74 = (texture2D (_MainTex, (tmpvar_2.xy + _ret_val_i0)) * _Color);
  vec3 tmpvar_75;
  tmpvar_75 = tmpvar_74.xyz;
  float tmpvar_76;
  tmpvar_76 = tmpvar_74.w;
  vec4 normal;
  normal.xy = ((texture2D (_BumpMap, (tmpvar_55 + _ret_val_i0)).wy * 2.0) - 1.0);
  normal.z = (vec3(sqrt (((1.0 - (normal.x * normal.x)) - (normal.y * normal.y))))).z;
  vec4 c_i0_i1;
  c_i0_i1.xyz = ((tmpvar_75 * _LightColor0.xyz) * (max (0.0, dot (normal.xyz, gl_TexCoord[2].xyz)) * 2.0)).xyz;
  c_i0_i1.w = (vec4(tmpvar_76)).w;
  c = c_i0_i1;
  c.xyz = (c_i0_i1.xyz + (tmpvar_75 * gl_TexCoord[3].xyz)).xyz;
  c.w = (vec4(tmpvar_76)).w;
...
void main ()
{
  ...
  vec4 c;
  vec2 tmpvar_48;
  tmpvar_48 = tmpvar_2.zw;
  vec2 _ret_val_i0;
  vec3 v;
  vec3 tmpvar_60;
  tmpvar_60 = normalize (gl_TexCoord[1].xyz);
  v = tmpvar_60;
  v.z = (vec3((tmpvar_60.z + 0.42))).z;
  _ret_val_i0 = (((texture2D (_ParallaxMap, tmpvar_48).w * _Parallax) - (_Parallax / 2.0)) * (v.xy / v.z));
  vec4 tmpvar_67;
  tmpvar_67 = (texture2D (_MainTex, (tmpvar_2.xy + _ret_val_i0)) * _Color);
  float tmpvar_69;
  tmpvar_69 = tmpvar_67.w;
  vec4 normal;
  normal.xy = ((texture2D (_BumpMap, (tmpvar_48 + _ret_val_i0)).wy * 2.0) - 1.0);
  normal.z = (vec3(sqrt (((1.0 - (normal.x * normal.x)) - (normal.y * normal.y))))).z;
  c.xyz = (tmpvar_67.xyz * (2.0 * texture2D (unity_Lightmap, gl_TexCoord[2].xy).xyz)).xyz;
  c.w = (vec4(tmpvar_69)).w;
  c.w = (vec4(tmpvar_69)).w;
  gl_FragData[0] = c.xyzw;
}
...
Program "fp" {
// Fragment combos: 2
//   opengl - ALU: 18 to 29, TEX: 3 to 3
//   d3d9 - ALU: 18 to 32, TEX: 3 to 3
SubProgram "opengl " {
Keywords { "DIRECTIONAL" "LIGHTMAP_OFF" }
...
void main ()
{
  ...
  vec4 c;
  vec2 tmpvar_54;
  tmpvar_54 = tmpvar_2.zw;
  vec2 _ret_val_i0;
  vec3 v;
  vec3 tmpvar_66;
  tmpvar_66 = normalize (gl_TexCoord[1].xyz);
  v = tmpvar_66;
  v.z = (vec3((tmpvar_66.z + 0.42))).z;
  _ret_val_i0 = (((texture2D (_ParallaxMap, tmpvar_54).w * _Parallax) - (_Parallax / 2.0)) * (v.xy / v.z));
  vec4 tmpvar_73;
  tmpvar_73 = (texture2D (_MainTex, (tmpvar_2.xy + _ret_val_i0)) * _Color);
  float tmpvar_75;
  tmpvar_75 = tmpvar_73.w;
  vec4 normal;
  normal.xy = ((texture2D (_BumpMap, (tmpvar_54 + _ret_val_i0)).wy * 2.0) - 1.0);
  normal.z = (vec3(sqrt (((1.0 - (normal.x * normal.x)) - (normal.y * normal.y))))).z;
  vec4 c_i0_i1;
  c_i0_i1.xyz = ((tmpvar_73.xyz * _LightColor0.xyz) * ((max (0.0, dot (normal.xyz, normalize (gl_TexCoord[2].xyz))) * texture2D (_LightTexture0, vec2(dot (tmpvar_8, tmpvar_8))).w) * 2.0)).xyz;
  c_i0_i1.w = (vec4(tmpvar_75)).w;
  c = c_i0_i1;
  c.w = (vec4(tmpvar_75)).w;
  gl_FragData[0] = c.xyzw;
}
...
void main ()
{
  ...
  vec4 c;
  vec2 tmpvar_52;
  tmpvar_52 = tmpvar_2.zw;
  vec2 _ret_val_i0;
  vec3 v;
  vec3 tmpvar_64;
  tmpvar_64 = normalize (gl_TexCoord[1].xyz);
  v = tmpvar_64;
  v.z = (vec3((tmpvar_64.z + 0.42))).z;
  _ret_val_i0 = (((texture2D (_ParallaxMap, tmpvar_52).w * _Parallax) - (_Parallax / 2.0)) * (v.xy / v.z));
  vec4 tmpvar_71;
  tmpvar_71 = (texture2D (_MainTex, (tmpvar_2.xy + _ret_val_i0)) * _Color);
  float tmpvar_73;
  tmpvar_73 = tmpvar_71.w;
  vec4 normal;
  normal.xy = ((texture2D (_BumpMap, (tmpvar_52 + _ret_val_i0)).wy * 2.0) - 1.0);
  normal.z = (vec3(sqrt (((1.0 - (normal.x * normal.x)) - (normal.y * normal.y))))).z;
  vec4 c_i0_i1;
  c_i0_i1.xyz = ((tmpvar_71.xyz * _LightColor0.xyz) * (max (0.0, dot (normal.xyz, gl_TexCoord[2].xyz)) * 2.0)).xyz;
  c_i0_i1.w = (vec4(tmpvar_73)).w;
  c = c_i0_i1;
  c.w = (vec4(tmpvar_73)).w;
  gl_FragData[0] = c.xyzw;
}
...
void main ()
{
  ...
  vec3 v;
  vec3 tmpvar_73;
  tmpvar_73 = normalize (gl_TexCoord[1].xyz);
  v = tmpvar_73;
  v.z = (vec3((tmpvar_73.z + 0.42))).z;
  _ret_val_i0 = (((texture2D (_ParallaxMap, tmpvar_61).w * _Parallax) - (_Parallax / 2.0)) * (v.xy / v.z));
  vec4 tmpvar_80;
  tmpvar_80 = (texture2D (_MainTex, (tmpvar_2.xy + _ret_val_i0)) * _Color);
  float tmpvar_82;
  tmpvar_82 = tmpvar_80.w;
  vec4 normal;
  ...
  vec4 c_i0_i1;
  c_i0_i1.xyz = ((tmpvar_80.xyz * _LightColor0.xyz) * ((max (0.0, dot (normal.xyz, normalize (gl_TexCoord[2].xyz))) * ((float((tmpvar_8.z > 0.0)) * texture2D (_LightTexture0, ((tmpvar_8.xy / tmpvar_8.w) + 0.5)).w) * texture2D (_LightTextureB0, vec2(dot (LightCoord_i0, LightCoord_i0))).w)) * 2.0)).xyz;
  c_i0_i1.w = (vec4(tmpvar_82)).w;
  c = c_i0_i1;
  c.w = (vec4(tmpvar_82)).w;
  gl_FragData[0] = c.xyzw;
}
...
void main ()
{
  ...
  normal.z = (vec3(sqrt (((1.0 - (normal.x * normal.x)) - (normal.y * normal.y))))).z;
  vec4 c_i0_i1;
  c_i0_i1.xyz = ((tmpvar_73.xyz * _LightColor0.xyz) * ((max (0.0, dot (normal.xyz, normalize (gl_TexCoord[2].xyz))) * (texture2D (_LightTextureB0, vec2(dot (tmpvar_8, tmpvar_8))).w * textureCube (_LightTexture0, tmpvar_8).w)) * 2.0)).xyz;
  c_i0_i1.w = (vec4(tmpvar_75)).w;
  c = c_i0_i1;
  c.w = (vec4(tmpvar_75)).w;
  gl_FragData[0] = c.xyzw;
...
void main ()
{
  ...
  normal.z = (vec3(sqrt (((1.0 - (normal.x * normal.x)) - (normal.y * normal.y))))).z;
  vec4 c_i0_i1;
  c_i0_i1.xyz = ((tmpvar_73.xyz * _LightColor0.xyz) * ((max (0.0, dot (normal.xyz, gl_TexCoord[2].xyz)) * texture2D (_LightTexture0, gl_TexCoord[3].xy).w) * 2.0)).xyz;
  c_i0_i1.w = (vec4(tmpvar_75)).w;
  c = c_i0_i1;
  c.w = (vec4(tmpvar_75)).w;
  gl_FragData[0] = c.xyzw;
...
Program "fp" {
// Fragment combos: 5
//   opengl - ALU: 27 to 39, TEX: 3 to 5
